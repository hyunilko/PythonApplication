...
def wait_intr_low(dev_gpio, mask: int, timeout_s: float = 5.0, poll_sleep_us: int = 10) -> int:
...
class SpiCaptureWorker(QObject):
    ...
    def start(self):
        ...
        try:
            ...
            set_device(self._dev_spi, self.settings.clock_hz, latency_timer=1, rw_timeout_ms=5000)
            ...
            if self.settings.save_to_file and self.settings.out_path:
                out_dir = os.path.dirname(self.settings.out_path)
                if out_dir:
            ...
            if self.settings.resync_on_start:
                ...
                while self._running:
                    ...
                    try:
                        wait_intr_low(self._dev_gpio, self.settings.host_intr_mask, timeout_s=0.25, poll_sleep_us=self.settings.poll_sleep_us)
                    except TimeoutError:
                        continue
            ...
            while self._running and frame_count < target_frames:
                ...
                if fout:
                    fout.write(chunk0)
                    frame_bytes_written += len(chunk0)
                ...
                while self._running and remain > 0:
                    ...
                    if fout:
                        fout.write(chunk)
                        frame_bytes_written += len(chunk)

                if not self._running:
                    break

                frame_count += 1
                total_bytes += frame_bytes_written
