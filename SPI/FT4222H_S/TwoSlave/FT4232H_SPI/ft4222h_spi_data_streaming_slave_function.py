...
이 모듈은 FT4222H를 SPI Slave로 사용하여 레이더 센서(SPI Master)로부터
...
def find_frame_sync_offset(data: bytes, debug: bool = False, max_search: int = 0) -> int:
    """
    APP_PDU 프레임 헤더 패턴을 기반으로 동기 오프셋을 찾습니다.

    :param data: 검색할 데이터
    :param debug: 디버그 출력 여부
    :param max_search: 최대 검색 범위 (0이면 전체 데이터 검색)
    :return: sync 패턴의 오프셋, 못 찾으면 -1
    """
    if len(data) < 16:
        return -1

    # 검색 범위 결정 (max_search가 0이면 전체 데이터 검색)
    search_limit = len(data) - 4 if max_search == 0 else min(len(data) - 4, max_search)

    # 빠른 검색: bytes.find 사용
    pattern = b'\x12\x34\x56\x78'
    offset = data.find(pattern, 0, search_limit + 4)

    if offset >= 0:
        if debug:
            print(f"[SYNC DEBUG] Found APP_PDU header at offset {offset}")
        return offset

    if debug:
        print(f"[SYNC DEBUG] APP_PDU pattern (12 34 56 78) not found in {search_limit} bytes")
                ...
                try:
                    if self.settings.variable_frame:
                        ...
                        if not check_frame_header_simple(header_data):
                            self.status_callback(f"[FRAME {frame_count+1}] Header mismatch, searching for sync...")
                            # 더 많은 데이터를 읽어서 sync 패턴 검색
                            search_buf = header_data
                            max_search_size = 64 * 1024  # 최대 64KB까지 검색
                            sync_found = False

                            while len(search_buf) < max_search_size and not sync_found:
                                # 추가 데이터 읽기
                                extra = spi_slave_read_once(self._spi, 4096)
                                if not extra:
                                    break
                                search_buf += extra

                                # sync 패턴 검색
                                offset = find_frame_sync_offset(search_buf)
                                if offset >= 0:
                                    # sync 찾음 - offset 위치부터 carry_over에 저장
                                    carry_over = search_buf[offset:]
                                    self.status_callback(f"[SYNC] Found header at offset {offset}, {len(carry_over)} bytes in carry_over")
                                    sync_found = True
                                    break

                            if not sync_found:
                                self.status_callback(f"[SYNC] Header not found in {len(search_buf)} bytes, flushing...")
...
if __name__ == "__main__":
            ...
            while not stop_flag.is_set():
                if msvcrt.kbhit():
                    ...
                    if key in (b'q', b'Q', b'\x03'):
                        print("\n[INFO] Stop key detected. Forcing exit...")
                        os._exit(0)
                time.sleep(0.05)
        except ImportError:
            pass
    ...
    print("[INFO] Press 'q' to stop capture...")
