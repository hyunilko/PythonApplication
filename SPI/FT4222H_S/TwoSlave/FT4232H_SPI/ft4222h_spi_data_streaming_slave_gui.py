...
class MainWindow(QMainWindow):
    ...
    def _log(self, msg: str) -> None:
        """
        GUI 로그 창(self.log_view)에 메시지를 타임스탬프와 함께 추가하고,
        커서를 문서 끝으로 이동시켜 항상 최신 로그가 보이도록 자동 스크롤을 유지합니다.

        Args:
            msg (str): 로그로 출력할 메시지(한 줄).
        """
        # 현재 시각을 "HH:MM:SS.mmm" (밀리초) 형식의 문자열로 생성
        # - %f는 마이크로초(6자리)까지 나오므로, 뒤 3자리를 잘라 밀리초로 맞춥니다.
        ts = datetime.now().strftime("%H:%M:%S.%f")[:-3]

        # QPlainTextEdit 끝에 한 줄 추가 (자동으로 개행 포함)
        self.log_view.appendPlainText(f"[{ts}] {msg}")

        # 현재 커서를 가져온 뒤, 문서 끝(End)으로 이동시켜 스크롤을 맨 아래로 내립니다.
        # - 사용자가 로그를 보고 있어도 새 로그가 추가될 때 최신 로그 위치로 따라가게 됩니다.
