...
사용 예:
  python swu_pc_tool.py --bin firmware.bin --model-id 0x32450001 --sw-version 0x00010002 --date 20260108 --block-size 65536
...
class SWUTool:
    def __init__(
        ...
        *,
        ...
        self.device_id = int(device_id) & 0x07  # 3-bit (0~7)
    ...
    def _make_can_id(self, msg_id: int) -> int:
        """CAN ID 생성: (DeviceID << 8) | MSG_ID"""
        return (self.device_id << 8) | (msg_id & 0xFF)
    ...
    def _wait_ack(self, expected_ack_id: int, *, timeout_s: float) -> Optional[bytes]:
        """
        CAN_ID=(DeviceID << 8) | 0x50 ACK 프레임을 기다립니다.
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                return None
            # CAN_ID 하위 8비트가 ACK MsgID(0x50)인지 확인
            if (can_id & 0xFF) != int(MsgID.ACK):
                continue
            try:
    ...
    def send_control(self, cmd_payload: bytes) -> None:
        """Control Request Frame: CAN_ID = (DeviceID << 8) | 0x32"""
        can_id = self._make_can_id(int(MsgID.CONTROL))
        self.mgr.send_bytes(can_id, cmd_payload)

    def send_verify(self, cmd_payload: bytes) -> None:
        """VERIFY_IMAGE also uses CONTROL CAN_ID = (DeviceID << 8) | 0x32"""
        can_id = self._make_can_id(int(MsgID.CONTROL))
        self.mgr.send_bytes(can_id, cmd_payload)

    def send_data_customtp(self, data_upper_msg: bytes) -> None:
        """DATA Frame: CustomTP segmentation over CAN_ID = (DeviceID << 8) | 0x33.
